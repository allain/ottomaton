...
Ottomaton.prototype = {
  ...
  register: function register(matcher, handler) {
    if (handler) {
  ...
  run: function run(lines, state) {
    ...
    return this._buildActions().then((function (actions) {
      ...
      if (unrecognizedLines.length) {
        return _nativePromiseOnly2['default'].reject(new LineError(unrecognizedLines));
      }
  ...
  _execute: function _execute(lines, state) {
    state = state || {};

    return _nativePromiseOnly2['default'].resolve(lines).then(reduce((function (state, line) {
      return this._executeLine(line, state);
    }).bind(this), state));
  },

  _executeLine: function _executeLine(line, state) {
    var recognized = false;
    var replacement = null;

    state.LINE = line;

    return _nativePromiseOnly2['default'].resolve(this._actions).then(reduce((function (state, action) {
      ...
      if (Array.isArray(args)) {
        ...
        if (typeof handler === 'function') {
          handlerResult = _nativePromiseOnly2['default'].resolve(handler.apply(state, args));
        } else if (typeof handler === 'string' || Array.isArray(handler)) {
          handlerResult = _nativePromiseOnly2['default'].resolve(handler);
        } else {
  ...
  _deref: function _deref(state, refs) {
  ...
  _buildActions: function _buildActions() {
    if (this._actions) return _nativePromiseOnly2['default'].resolve(this._actions);
    ...
    return _nativePromiseOnly2['default'].all(this.registrations.map(expandAction)).then(flatten).then((function (actions) {
