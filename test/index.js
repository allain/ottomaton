...
test('core - can be created using constructor', function (t) {
  ...
  t.equal(typeof otto.run, 'function', 'ottomaton has run method')
  t.equal(typeof otto.register, 'function', 'ottomaton has register method')
  t.strictEqual(otto.opts, opts, 'ottomaton exposes opts')
  t.end()
})
...
test('core - can disable dereferencing and then call deref inside action', function (t) {
  Ottomaton({common: false}).register(Action(/^(.*)$/, function (varName) {
    t.equal(varName, 'VAR', 'expected first argument to be first regex match')
    t.equal(this.deref(varName), 'Hello!')
    t.end()
  }, {deref: false})).run(['VAR'], {VAR: 'Hello!'}).catch(t.end)
})

test('core - dereferencing to a value that looks like a ref does not further dereference', function (t) {
...
test('core - nothing is executed if a line does not have any actions', function (t) {
  return Ottomaton().register({
    ...
    t.equal(err.message, 'Unrecognized Line: huh?')
  })
})
...
test('core - implicitly adds a FINISH line at end of scripts', function (t) {
  var ottomaton = Ottomaton({common: false})
